...
void solve(){
    ...
    // For each starting point of the subarray
    for (int i = 0; i < n; ++i) {
        unordered_set<int> seen; // To track distinct elements
        int min_val = a[i], max_val = a[i];
        
        // Extend the subarray to the right
        for (int j = i; j < n; ++j) {
            // If the element is already in the set, break (no longer distinct)
            if (seen.count(a[j])) {
                break;
            }
            ...
            // Check if the subarray [i..j] is brilliant
