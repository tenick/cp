...
int top_down_way(int x, const vector<int>& coins) {
    ...
    int ans = INT_MAX;
    for (int coin : coins) {
        int ans1 = top_down_way(x - coin, coins);
        ans = min(ans, ans1 == INT_MAX ? INT_MAX : ans1 + 1);
    }
    return ans;
}
...
int top_down_way_memo(int x, vector<int>& memo, const vector<int>& coins) {
    ...
    int ans = INT_MAX;
    for (int coin : coins) {
        int ans1 = top_down_way_memo(x - coin, memo, coins);
        ans = min(ans, ans1 == INT_MAX ? INT_MAX : ans1  + 1);
    }
    return memo[x] = min(memo[x], ans);
}
...
void solve(){
    ...
    //int ans = top_down_way_memo(x, memo, coins);
    int ans = bottom_up_way(x, coins);
